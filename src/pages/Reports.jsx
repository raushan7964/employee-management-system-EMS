...
const Reports = ({ userRole, currentEmployee }) => {
  ...
  const loadReportsData = () => {
    ...
    if (userRole === 'employee' && currentEmployee?.email) {
      tasks = tasks.filter(task => task.assignee === currentEmployee.email)
    }
    ...
    if (userRole === 'admin') {
      ...
      const allTasks = getTasks() // Use all tasks for performance calculation
      
      const performanceMap = {}
      allTasks.forEach(task => {
        if (task.status === 'completed') {
          if (!performanceMap[task.assignee]) {
            performanceMap[task.assignee] = {
              email: task.assignee,
          ...
          performanceMap[task.assignee].completed++
        }
        if (!performanceMap[task.assignee]) {
          performanceMap[task.assignee] = {
            email: task.assignee,
        ...
        performanceMap[task.assignee].total++
      })
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className={`grid grid-cols-1 ${userRole === 'admin' ? 'lg:grid-cols-2' : ''} gap-8 mb-8`}>
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between text-sm mb-2">
                ...
                <span className="font-bold text-slate-800">{stats.completed} ({completionRate}%)</span>
              </div>
            ...
            <div>
              <div className="flex justify-between text-sm mb-2">
                ...
                <span className="font-bold text-slate-800">{stats.inProgress} ({stats.total > 0 ? ((stats.inProgress / stats.total) * 100).toFixed(1) : 0}%)</span>
              </div>
              <div className="h-4 bg-slate-100 rounded-full overflow-hidden">
                <div 
            ...
            <div>
              <div className="flex justify-between text-sm mb-2">
                ...
                <span className="font-bold text-slate-800">{stats.new} ({stats.total > 0 ? ((stats.new / stats.total) * 100).toFixed(1) : 0}%)</span>
              </div>
              <div className="h-4 bg-slate-100 rounded-full overflow-hidden">
                <div 
            ...
            <div>
              <div className="flex justify-between text-sm mb-2">
                ...
                <span className="font-bold text-slate-800">{stats.failed} ({stats.total > 0 ? ((stats.failed / stats.total) * 100).toFixed(1) : 0}%)</span>
              </div>
              <div className="h-4 bg-slate-100 rounded-full overflow-hidden">
                <div 
                ...
                {topPerformers.map((performer, idx) => (
                  <div key={idx} className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-slate-500">{performer.completed} completed / {performer.total} total</div>
                    </div>
                    <div className="text-right">
                      <div className="text-lg font-bold text-indigo-600">{performer.completionRate.toFixed(0)}%</div>
              ...
              <div key={idx} className="flex items-center gap-4 p-3 hover:bg-slate-50 rounded-lg transition-colors">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-slate-500">
                    {task.assigneeName} • {new Date(task.updatedAt).toLocaleDateString()}
                  </div>
                </div>
