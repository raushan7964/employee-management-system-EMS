...
const AdminDashboard = () => {
  ...
  const loadPendingApprovals = () => {
    ...
    const pending = tasks.filter(task => 
      task.approvalStatus === 'pending' || task.status === 'pending-approval'
    )
          ...
          {pendingTasks.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {pendingTasks.map((task) => (
                <div key={task.id} className="bg-white rounded-lg shadow-md p-4 border-2 border-orange-200">
                  <div className="mb-3">
                  ...
                  <div className="mb-3 pb-3 border-b border-slate-100">
                    ...
                    <div className="flex items-center gap-2 text-sm mt-1">
                      ...
                      <span className="font-semibold text-slate-800">{new Date(task.dueDate).toLocaleDateString()}</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm mt-1">
          ...
          {employeeStats.length === 0 ? (
            ...
            <div className="space-y-6">
              {employeeStats.map((emp, idx) => {
                const completionRate = emp.total > 0 ? ((emp.completed / emp.total) * 100).toFixed(0) : 0
